...
//  useCompactVMConfiguration();
  ...
  String testApplicationPath =
      getTestPagePath("bug_example/value", "application.html");

  setUp(() {
    return getWebDriver()
        .then((_driver) => driver = _driver)
        .then((_) => driver.get(testApplicationPath))
        .then((_) => driver.findElement(new By.cssSelector('input[type=text]')))
        .then((_element) => textInput = _element)
        .then((_) => driver.findElement(new By.cssSelector('label')))
  ...
  group("webdriver", () {
    seleniumTest('should be possible to write to input', () {
      return driver.keyboard
          ...
          .then((_) => textInput.attributes['value']).then((value) {
        expect(value, 'abcdef');
      });
    });
    ...
    seleniumTest('should fill span with value from input with cursor move', () {
      return driver.keyboard
          .sendKeys(['a', 'e', 'f', Keys.LEFT, Keys.LEFT, 'b', 'c', 'd'])
          .then((_) => sleep(100)) //because of animation frame
          .then((_) => label.text).then((text) {
        expect(text, 'abcdef');
      });
